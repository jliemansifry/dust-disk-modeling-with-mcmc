...
def findBestFit():
	...
	visGenBestFit = np.ravel([45, 45]) # set inc amd PA
...
def triangleGen():
	...
	triangle.corner(samples, labels=['$R_{In}$ [AU]','$\Delta$R [AU]','log($M_{D}$ [$M_{Earth}$])'], truths=[bestDisk[0],bestDisk[1],bestDisk[2]], truth_color='g', quantiles=[0.16,.5,.84], show_titles=True, plot_contours=True)
